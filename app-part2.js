...
async function duplicateLastFD() {
    ...
    const enabledRecords = records.filter(record => {
        const holder = holders.find(h => {
            const holderName = typeof h === 'object' ? h.name : h;
            return holderName === record.accountHolder;
        });
    ...
    const lastRecord = enabledRecords[enabledRecords.length - 1];
